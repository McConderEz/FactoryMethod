...
{
    public class WarriorHero : HeroBase
    {
        public WarriorHero(string name, int health, int damage, int protection, int agility)
        ...
        {
            Console.WriteLine($"{Name} совершил атаку мечём и нанёс {Damage} урона!");
        }

        public override ISkill CreateSkill(string skillName, double cdTime, int manaCost)
        {
            var skill = new WarriorSkill(skillName, cdTime, manaCost, Name);
            return skill;
        }
